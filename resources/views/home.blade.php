        ...
        <div class="bg-white shadow-md rounded-lg">
            ...
            <div class="p-6">
                <form action="{{ route('upload') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    <div>
                        <label for="fileUpload" class="block text-sm font-medium text-gray-700 mb-2">Выберите файл</label>
                        <input type="file" name="file" id="fileUpload" class="block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required>
                        <p class="mt-1 text-sm text-gray-500">Доступные форматы: .xlsx, .csv</p>
                    </div>
                    <div>
                        <label for="reference_id" class="block text-sm font-medium text-gray-700 mb-2">Идентификатор источника</label>
                        <input name="reference_id" id="reference_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" required>
                    </div>
...
{{--                                </td>--}}
                                ...
                                <td class="px-4 py-2">
                                    @if($upload->is_executed === 1)
                                        <span class="inline-block bg-green-300 px-3 py-1 rounded-full text-xs font-medium">
                                            Обработан
                                        </span>
                                    @else
                                        <span class="inline-block bg-yellow-300 px-3 py-1 rounded-full text-xs font-medium">
                                        Не обработан
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-2 text-left">
                                    <a href="{{ route('clients', $upload->hash) }}" class="bg-blue-500 text-white px-3 py-1 rounded-md text-xs font-medium hover:bg-blue-600 focus:outline-none focus:ring focus:ring-blue-300 inline-block">
